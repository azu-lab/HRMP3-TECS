...
#  の責任を負わない．
# 
#  $Id: testcfg.rb 1060 2023-01-08 13:38:16Z ertl-hiro $
# 
...
#	0: 自動メモリ配置
#	1: 手動メモリ配置
#	2: USE_LATERPASS_DOMINIB，自動メモリ配置
#
CFG_TEST_SPEC = {
  # ビルドに成功するテスト
  "cfg_all1"   => { },
  ...
  "cfg_mml1"      => { TARGET: 1, ML_MANUAL: true },
  "cfg_dominib"   => { TARGET: 2 },
}

WARNING_TEST_SPEC = {
  # 警告は出るがビルドに成功するテスト
  "cfg_warning1"  => { DEFS: "-DOMIT_TARGET_MEMCFG" },
  "cfg_warning2"  => { },
}
...
def BuildTest(test, testSpec, errorRedirect, mkdirFlag=false, rmFlag=false)
  ...
  Dir.chdir(objDir) do
    ...
    # コンフィギュレータが先に実行されるようにする
    makeCommand = "make kernel_cfg.c all"
    if errorRedirect
...
def CfgTest(test, testSpec)
  ...
  Dir.chdir(objDir) do
    ...
    diffFile(test, "kernel_mem.c", objDir)
    diffFile(test, "ldscript.ld", objDir) unless testSpec.has_key?(:ML_MANUAL)
  end
end
...
ARGV.each do |param|
  ...
  when "all"
    ...
    if CFG_TEST_SPEC.has_key?(param)
      ...
      if ($clean_flag)
        CleanTest(param, WARNING_TEST_SPEC[param])
      else
        BuildTest(param, WARNING_TEST_SPEC[param], true, true, true) \
															unless $exec_only
        ErrorTest(param, WARNING_TEST_SPEC[param]) unless $build_only
      end
    elsif PASS4_TEST_SPEC.has_key?(param)
      if ($clean_flag)
        CleanTest(param, PASS4_TEST_SPEC[param])
      else
        BuildTest(param, PASS4_TEST_SPEC[param], true, true) unless $exec_only
        ErrorTest(param, PASS4_TEST_SPEC[param]) unless $build_only
      end
