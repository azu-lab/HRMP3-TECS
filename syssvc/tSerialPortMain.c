 ...
 *  の責任を負わない．
 * 
 *  $Id: tSerialPortMain.c 3234 2021-10-24 13:13:34Z ertl-hiro $
 */
...
serialPort_readChar(CELLCB *p_cellcb, char *p_c)
{
	...
	*p_c = VAR_receiveBuffer[VAR_receiveReadPointer];
	...
	VAR_receiveCount--;
	static volatile int rCount = -100;
	rCount = VAR_receiveCount;
	syslog(LOG_EMERG, "%s:%d rCount = %d", __FILE__ , __LINE__, rCount );
...
//	syslog(LOG_NOTICE,"%d",buffer_empty);
	if (!buffer_empty) {
//		SVC(cReceiveSemaphore_signal(), gen_ercd_sys(p_cellcb));

		ER sample1 = cReceiveSemaphore_signal();
		if(sample1 == E_CTX){
			syslog(LOG_EMERG, "%s:%d E_CTX", __FILE__ , __LINE__);
		}
		else if(sample1 == E_ID){
			syslog(LOG_EMERG, "%s:%d E_ID", __FILE__ , __LINE__);
		}
		else if(sample1 == E_NOEXS){
			syslog(LOG_EMERG, "%s:%d E_NOEXS", __FILE__ , __LINE__);
		}
		else if(sample1 == E_OACV){
			syslog(LOG_EMERG, "%s:%d E_OACV", __FILE__ , __LINE__);
		}
		else if(sample1 == E_QOVR){
//			syslog(LOG_EMERG, "%s:%d E_QOVR", __FILE__ , __LINE__);
			syslog(LOG_EMERG, "E_QOVR:Semaphore");
		}
...
eiSIOCBR_readySend(CELLIDX idx)
{
	CELLCB	*p_cellcb;
//	bool_t checkFlag;
//	bool_t checkFlag = VAR_errorFlag;

	assert(VALID_IDX(idx));
	p_cellcb = GET_CELLCB(idx);
//	checkFlag = VAR_errorFlag;
	if (VAR_receiveFlowControl != '\0') {
		...
		if (VAR_sendCount == ATTR_sendBufferSize) {
			...
			/*
			ER sample1 = ciSendSemaphore_signal();
			if(sample1 == E_CTX){
				syslog(LOG_EMERG, "%s:%d E_CTX", __FILE__ , __LINE__);
			}
			else if(sample1 == E_ID){
...
eiSIOCBR_readyReceive(CELLIDX idx)
{
	...
	if ((VAR_ioControl & IOCTL_FCSND) != 0U && c == FC_STOP) {
		...
		if (VAR_receiveCount == 0U) {
			...
			/*
			ER sample1 = ciReceiveSemaphore_signal();
			if(sample1 < 0) {
				if(sample1 == E_CTX){
					syslog(LOG_EMERG, "%s:%d E_CTX", __FILE__ , __LINE__);
				}
		...
		static volatile int rCount = -200;
		VAR_receiveCount++;
		rCount = VAR_receiveCount;
		ID prcid;
		get_pid(&prcid);
		syslog(LOG_EMERG, "%s:%d rCount = %d in proc %d, char = %d\n", __FILE__ , __LINE__, rCount, prcid , c );
