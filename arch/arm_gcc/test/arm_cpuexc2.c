 ...
 *  の責任を負わない．
 * 
 *  $Id: arm_cpuexc2.c 866 2021-02-22 06:17:19Z ertl-hiro $
 */
 ...
 * 【テストの目的】
 *
 *  ARMコア依存部のCPU例外処理をテストする．ユーザタスクからの未定義命
 *  令，プリフェッチアボート，データアボートに対して，CPU例外処理のテ
 *  スト(6)とCPU例外処理のテスト(5)の状況を連続してテストする．また，
 *  システムタスクであればフェイタルデータアボートになる状況で，通常の
 *  CPU例外になることをテストする．
 *
 *  なお，HRP3カーネルでは，サービスコール呼出しにsvc命令を用いている
 *  ため，SVCによるCPU例外処理のテストは行わない．
 *
 * 【テスト項目】
 *
 *  未定義命令，プリフェッチアボート，データアボートに対して，以下の
 *  (1)と(2)をテストする．
 *
 ...
 *      込み優先度マスク全解除状態，ディスパッチ許可状態で，システムタ
 *      スクであればフェイタルデータアボートになる状況を作り，発生した
 *      CPU例外において，以下を確認する．
 *
 *	(3-A) CPU例外ハンドラにおいて，CPUロック解除状態になっていること
 *	(3-B) CPU例外ハンドラにおいて，xsns_dpnがfalseを返すこと
 *	(3-C) タスク切換えによるリカバリができること
 *
 * 【使用リソース】
 *
 ...
 *		ipm(TIPM_ENAALL)
 *		DO(RAISE_CPU_EXCEPTION_PABORT_USR)
 *	== CPUEXC3-1（1回目）==
 ...
 *		assert(xsns_dpn(p_excinf) == true)					... (2-G)
 *	23:	DO(PREPARE_RETURN_CPUEXC_PABORT_USR)
 *		RETURN
 *	== TASK1-3（続き）==
 ...
 *		assert(xsns_dpn(p_excinf) == true)					... (2-G)
 *	35:	DO(PREPARE_RETURN_CPUEXC_DABORT)
 *		RETURN
 *	== TASK1-4（続き）==
 ...
 *		ena_dsp()
 *
 *	// システムタスクであればフェイタルデータアボートになる状況
 *	37:	state(false, false, false, false, false)
 *		ipm(TIPM_ENAALL)
 *		DO(RAISE_CPU_EXCEPTION_FATAL)
 *	== CPUEXC1-3（3回目）==
 *	38:	state(true, false, false, true, false)				... (3-A)
 *		assert(xsns_dpn(p_excinf) == false)					... (3-B)
 *  39:	act_tsk(TASK2)
 *		RETURN
 *	== TASK2-4（4回目）==
 *	40:	ter_tsk(TASK1)										... (3-C)
 *	41:	act_tsk(TASK1)										... (3-C)
 *	42:	ext_tsk()
 *	== TASK1-5（5回目）==
 *	43:	state(false, false, false, false, false)			... (3-C)
 *	44:	END
 */
...
cpuexc3_handler(void *p_excinf)
{
	...
	case 2:
		...
		check_point(23);
		PREPARE_RETURN_CPUEXC_PABORT_USR;
...
task1(EXINF exinf)
{
	...
	case 2:
		...
		RAISE_CPU_EXCEPTION_PABORT_USR;
