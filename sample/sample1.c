 ...
 *  の責任を負わない．
 * 
 *  $Id: sample1.c 990 2022-11-20 11:48:51Z ertl-hiro $
 */
...
char *const	task_name[] = { "TASK1_1", "TASK1_2", "TASK1_3",
							"TASK2_1", "TASK2_2", "TASK2_3" };
char *const	task_graph[] = { " @   |   ||     |",
							 "   + |   ||     |",
							 "     | * ||     |",
							 "     |   || %   |",
							 "     |   ||   # |",
...
main_task(EXINF exinf)
{
	...
	// syslog(LOG_EMERG, "Sample program starts (exinf = %d).", (int_t) exinf);
	/*
	 *  シリアルポートの初期化
	 *
	...
	SYSTIM  start_o, end_o, overhead;
	get_tim(&start_o);
    for(int_t i=0; i<10; i++)
    	get_tim(&end_o);
    overhead = (end_o - start_o)/10;
