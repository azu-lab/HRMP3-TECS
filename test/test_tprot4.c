 ...
 *  の責任を負わない．
 * 
 *  $Id: test_tprot4.c 1051 2022-12-28 05:30:31Z ertl-hiro $
 */
 ...
 *		  を割当て
 *	DOM3: タイムウィンドウを割り当てない→アイドルウィンドウで実行
 *	DOM4: タイムウィンドウを割り当てない→アイドルウィンドウで実行
 *
 ...
 *	CYC13: DOM1，SEM1にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC14: DOM1，SEM1にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC15: DOM1，SEM1にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC31: DOM3，SEM3にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC32: DOM3，SEM3にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC33: DOM3，SEM3にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 *	CYC34: DOM3，SEM3にsignal，初期起動：0，周期：1（過負荷な周期ハンドラ）
 ...
 *	FLG1: 無所属，初期値：0x00
 *	SEM1: 無所属，初期値：0，最大値：UINT_MAX
 *	SEM3: 無所属，初期値：0，最大値：UINT_MAX
 *
 * 【テストシーケンス】
 *
 *	// タイムウィンドウ for DOM1
 *	// CYC11が実行される（TASK12が起動される）
 *	== TASK12-1 ==
 *	1:	ext_tsk()
 *	== TASK11 ==
 *	2:	WAIT(task11_flag)
 ...
 *	// ALM5が実行される（FLG1の0x10がセットされる）				... (A-3)
 *	7:	assert(flgptn == 0x90U)
 *		DO(flgptn = poll_flag_change(FLG1, flgptn))
 *	// タイムウィンドウ for DOM2
 *	// ALM2が実行される（FLG1の0x02がセットされる）				... (A-1)
 *	== TASK2 ==
 *	8:	pol_flg(FLG1, 0xffU, TWF_ORW, &flgptn)
 *		assert(flgptn == 0x92U)
 *		DO(flgptn = poll_flag_change(FLG1, flgptn))
 *	// アイドルウィンドウ
 *	// ALM3とALM4が実行される（FLG1の0x04と0x08がセットされる）	... (A-2)
 *	== TASK3 ==
 ...
 *	// タイムウィンドウ for DOM1
 *	// CYC11が実行される（TASK12が起動される）
 *	// ALM1が実行される（FLG1の0x01がセットされる）				... (A-1)
 *	== TASK12-3 ==
 *	10:	ext_tsk()
 *	== TASK11 ==
 ...
 *		WAIT(task3_flag)
 *	// タイムウィンドウ for DOM1
 *	// CYC12〜CYC15が実行される．これらがタイムウィンドウを使い切る	... (B-1)
 *	// CYC11は実行されない．
 ...
 *		WAIT(task2_flag)
 *	// アイドルウィンドウ
 *	// CYC31〜CYC34が実行される．これらがタイムウィンドウを使い切る	... (B-2)
 *	// TASK3が一瞬実行される可能性がある
 *	== TASK3 ==
 *		WAIT_WO_RESET(task3_flag_1)
 *	// タイムウィンドウ for DOM1
 *	// CYC11が2回実行される（TASK12が2回起動される）
 *	== TASK12-4 ==
