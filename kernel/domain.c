 ...
 *  の責任を負わない．
 * 
 *  $Id: domain.c 1027 2022-11-28 11:00:11Z ertl-hiro $
 */
...
initialize_schedcb(SCHEDCB *p_schedcb)
{
	...
	if (p_schedcb != NULL) {
		...
		for (i = 0; i < TNUM_TPRI; i++) {
			...
			// for(k=0; k<10000; k++);
			// syslog(LOG_EMERG, "%d\n", i);
//			syslog(LOG_EMERG, "%d %p %d\n", i, &i, TNUM_TPRI);
		// queue_initialize(&(p_schedcb->ready_queue[0]));
		// queue_initialize(&(p_schedcb->ready_queue[1]));
		// queue_initialize(&(p_schedcb->ready_queue[2]));
		// queue_initialize(&(p_schedcb->ready_queue[3]));
		// queue_initialize(&(p_schedcb->ready_queue[4]));
		// queue_initialize(&(p_schedcb->ready_queue[5]));
		// queue_initialize(&(p_schedcb->ready_queue[6]));
		// queue_initialize(&(p_schedcb->ready_queue[7]));
		// queue_initialize(&(p_schedcb->ready_queue[8]));
		// queue_initialize(&(p_schedcb->ready_queue[9]));
		// queue_initialize(&(p_schedcb->ready_queue[10]));
		// queue_initialize(&(p_schedcb->ready_queue[11]));
		// queue_initialize(&(p_schedcb->ready_queue[12]));
		// queue_initialize(&(p_schedcb->ready_queue[13]));
		// queue_initialize(&(p_schedcb->ready_queue[14]));
		// queue_initialize(&(p_schedcb->ready_queue[15]));
		}
...
initialize_domain(PCB *p_my_pcb)
{
	...
	if (system_cyctim == 0U) {
		 ...
		 *  時間パーティショニングを使用する場合
		 */
		p_my_pcb->scyc_tmevtb.callback = (CBACK) scyc_switch;
		p_my_pcb->scyc_tmevtb.arg = (void *) NULL;
		...
		if (p_inisom != NULL) {
			 ...
			 */
			p_my_pcb->scyc_tmevtb.evttim = 0;
			tmevtb_register(&(p_my_pcb->scyc_tmevtb),
...
scyc_start(PCB *p_my_pcb)
{
	...
	if (p_my_pcb->p_cursom == NULL) {
		 ...
		 *  システム周期の開始
		 */
		p_my_pcb->scyc_tmevtb.evttim += system_cyctim;
		tmevtb_enqueue(&(p_my_pcb->scyc_tmevtb), p_my_pcb->p_tmevt_heap_kernel,
																	p_my_pcb);
...
void
scyc_switch(PCB *p_my_pcb)
{
	if (p_my_pcb->dspflg) {
		scyc_start(p_my_pcb);
		update_schedtsk(p_my_pcb);
...
suspend_proc_tmevt(PCB *p_my_pcb)
{
	if (p_my_pcb->twdtimer_enable) {
		 ...
		 *  システム周期切換え時刻になっているか？
		 */
		return(tmevt_lefttim(&(p_my_pcb->scyc_tmevtb)) == 0U);
	}
}
...
twd_start(PCB *p_my_pcb)
{
	...
	if (p_tmevt_heap != NULL) {
		while (!suspend_proc_tmevt(p_my_pcb)
							&& tmevt_proc_top(p_my_pcb, p_tmevt_heap)) ;
	}
}
...
twd_switch(void)
{
	...
	if (p_my_pcb->twdtimer_enable && p_my_pcb->left_twdtim == 0U) {
		 ...
		 *  キャンセルされた場合
		 */
		syslog_2(LOG_NOTICE, "spurious twd interrupt "
...
chg_som(ID somid)
{
	...
	CHECK_ACPTN(sysstat1_acvct.acptn1, selfdom);	/*［NGKI5034］*/
	...
	if (p_global_cursom == NULL && p_sominib != NULL) {
		...
		p_my_pcb->scyc_tmevtb.evttim = calc_current_evttim_ub();

		/*
		 *  他のプロセッサに割込みで知らせる
		 */
		scyc_evttim = p_my_pcb->scyc_tmevtb.evttim;
...
start_scyc_handler(void)
{
	 ...
	 *  基準時刻の設定
	 */
	p_my_pcb->scyc_tmevtb.evttim = scyc_evttim;
...
get_som(ID *p_somid)
{
	...
	CHECK_ACPTN(sysstat1_acvct.acptn4, selfdom);		/*［NGKI5063］*/
