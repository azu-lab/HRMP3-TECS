 ...
 *  の責任を負わない．
 * 
 *  $Id: semaphore.c 1064 2023-01-08 15:18:39Z ertl-hiro $
 */
...
sig_sem(ID semid)
{
	...
	CHECK_UNL_MYSTATE(&p_selftsk, &selfdom, &context);	/*［NGKI1501］*/
	CHECK_ID(VALID_SEMID(semid));				/*［NGKI1502］*/
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn1, selfdom);	/*［NGKI1504］*/
...
wai_sem(ID semid)
{
	...
	CHECK_DISPATCH_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1515］*/
	CHECK_ID(VALID_SEMID(semid));				/*［NGKI1517］*/
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn2, selfdom);	/*［NGKI1520］*/
...
pol_sem(ID semid)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1513］［NGKI1514］*/
	CHECK_ID(VALID_SEMID(semid));				/*［NGKI1517］*/
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn2, selfdom);	/*［NGKI1520］*/
...
ini_sem(ID semid)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1527］［NGKI1528］*/
	CHECK_ID(VALID_SEMID(semid));				/*［NGKI1529］*/
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn3, selfdom);	/*［NGKI1531］*/
...
ref_sem(ID semid, T_RSEM *pk_rsem)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1536］［NGKI1537］*/
	CHECK_ID(VALID_SEMID(semid));				/*［NGKI1538］*/
	CHECK_MACV_WRITE(pk_rsem, T_RSEM, p_selftsk, selfdom);	/*［NGKI1541］*/
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn4, selfdom);	/*［NGKI1540］*/
