 ...
 *  の責任を負わない．
 * 
 *  $Id: mem_manage.c 1060 2023-01-08 13:38:16Z ertl-hiro $
 */
...
/*
 *  メモリオブジェクトの状態参照（ref_memの本体）
 */
#ifndef OMIT_REFER_MEMORY

Inline ER
refer_memory(const void *addr, ACPTN selfdom, T_RMEM *pk_rmem)
{
	int_t	meminib;
	ATR		accatr;
	ER		ercd;
...
ref_mem(const void *addr, T_RMEM *pk_rmem)
{
	...
	ercd = refer_memory(addr, selfdom, pk_rmem);
