 ...
 *  の責任を負わない．
 * 
 *  $Id: sys_manage.c 898 2022-06-21 12:30:39Z ertl-hiro $
 */
...
rot_rdq(PRI tskpri)
{
	...
	if (!context) {								/*［NGKI3766］*/
		CHECK_ACPTN(p_selftsk->p_dominib->acvct.acptn2, selfdom);
	}
...
mrot_rdq(ID schedno, PRI tskpri)
{
	...
	CHECK_ACPTN(p_dominib->acvct.acptn2, selfdom);		/*［NGKI3778］*/
...
loc_cpu(void)
{
	...
	LOG_LOC_CPU_ENTER();
	CHECK_ACPTN(sysstat1_acvct.acptn2, get_selfdom());	/*［NGKI2729］*/
...
unl_cpu(void)
{
	...
	LOG_UNL_CPU_ENTER();
	CHECK_ACPTN(sysstat1_acvct.acptn2, get_selfdom());	/*［NGKI2736］*/
...
dis_dsp(void)
{
	...
	CHECK_ACPTN(sysstat1_acvct.acptn1, selfdom);	/*［NGKI2743］*/
...
ena_dsp(void)
{
	...
	CHECK_ACPTN(sysstat1_acvct.acptn1, selfdom);	/*［NGKI2749］*/
