...
class HRMPKernelObjectPlugin < CelltypePlugin
  ...
  def gen_factory file
    ...
    # 時間パーティショニング設定用のJSONテンプレート生成
    require 'json'
    user_array = Array.new
    if !File.exist?("#{$gen}/TWD.json")
      # 出力用のハッシュ
      output_hash = {}
      output_hash[:SystemCycleTime] ||= 10000
      output_hash['SOM'] ||= []
      output_hash['SOM'][0] ||= {}
      output_hash['SOM'][0][:name] ||= "SOM1"
      output_hash['SOM'][0][:next] ||= "SOM1"
    else
      json_file = File.read("#{$gen}/TWD.json")
      output_hash = JSON.parse(json_file)
      # "CLS_"から始まるセクションの変数を配列に格納
      output_hash['SOM'].each do |som|
        som.each do |key, value|
          if key.start_with?('CLS_')
            value.each do |k, v|
              if k.start_with?('rUserDomain')
                user_array << k
              end
            end
          end
    ...
    @cell_list.each{ |cell|
      ...
      doption = domain_type.get_option
        ...
        unless output_hash['SOM'].any? { |som| som.key?(coption) }
          #CLS_ALLのときは新たなSOMセクションを作成
          if coption.start_with?("CLS_ALL_")
            output_hash['SOM'][1] ||= {}
            output_hash['SOM'][1][:name] ||= "SOM2"
            output_hash['SOM'][1][:next] ||= "SOM2"
            output_hash['SOM'][1][coption] ||= {}
          else
            output_hash['SOM'][0][coption] ||= {}
          end
        end

      # doption == "user"の際にdroot.get_nameで文字列を出力
      if doption == "user"
        user_array << droot.get_name
      end
    }
    user_array.uniq!

    # ユーザドメインを各クラスのセクションに格納
    user_array.each do |user|
      output_hash['SOM'].each do |som|
        som.each do |key, value|
          if key.to_s.start_with?("CLS_")
            som[key][user] ||= 1000
          end
        end
      end
    ...
    # 出力ファイルにJSONを書き込み
    File.open("#{$gen}/TWD.json", 'w') do |file|
